...
/*
    1. 内存4区模型
        ...
        2) 数据段: .rodata段(只读数据段), .data段(初始胡数据段), .bss段(未初始化数据段) -- 存放全局变量、静态变量以及常量
            a) .data段(初始化数据段): 初始化为非0的 全局变量和静态变量
        ...
        4) heap: 堆. 给用户自定义数据提供空间. 空间大小约为 1.3G+
            空间足够大, 由程序员分配、释放, 若程序员不释放, 程序结束时由操作系统回收
...
{
    int* pt = func1(); // 接收局部变量的地址(栈)
    cout << *pt << endl; // 10 第一次可以打印正确数据, 是因为编译器做了保留
    cout << *pt << endl; // xxxx 第二次这个数据就不再保留了, 输出乱码

    pt = func2(); // 接收new出来的地址(堆)
    cout << *pt << endl; // 10 第一次可以打印正确数据
    cout << *pt << endl; //10 第二次仍可以打印正确数据
    delete pt;
    // cout << *pt << endl; // 运行时会报错: 空指针
